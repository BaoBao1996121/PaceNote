...
2. **包含服务前缀**: 预期 MCP 服务器可能与其他 MCP 服务器一起使用
...
**注意**: stdio 服务器不应记录到 stdout（使用 stderr 记录日志）
...
- 检查参数大小和范围
- 防止系统调用中的命令注入
- 使用 schema 验证（Pydantic/Zod）所有输入
